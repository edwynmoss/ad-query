...
function InspectorBody({ entry, onClose }: { entry: ldap.Entry; onClose: () => void }) {
  ...
  return (
    <div className="w-[360px] shrink-0 flex flex-col min-h-0" style={{ borderLeft: "1px solid var(--color-line)", background: "var(--color-canvas)" }}>
      <div className="flex items-start justify-between gap-2 px-3 py-2.5" style={{ borderBottom: "1px solid var(--color-line)" }}>
        ...
        <button className="btn btn-quiet btn-icon shrink-0" onClick={onClose} aria-label="close"><X size={15} /></button>
      </div>

      <div className="tabs px-3 gap-4" style={{ borderBottom: "1px solid var(--color-line)" }}>
        <button className="tab" data-active={tab === "attrs"} onClick={() => setTab("attrs")}>Attributes</button>
        <button className="tab" data-active={tab === "acl"} onClick={() => setTab("acl")}>Security</button>
      </div>
  ...
  if (status === "idle") {
    return (
      <div className="space-y-2">
        ...
        <button className="btn h-7" onClick={load}>Load security descriptor</button>
      </div>
    );
  }
  ...
  return (
    <div className="p-1.5 rounded" style={{ border: "1px solid var(--color-line)", background: "var(--color-surface)" }}>
      <div className="flex items-center gap-1.5">
        <span className={"status " + (ace.allow ? "status-ok" : "status-danger")}>{ace.allow ? "ALLOW" : "DENY"}</span>
      ...
      {ace.rights && ace.rights.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-1">
          {ace.rights.map((r, i) => <span key={i} className="token h-[16px] text-[10.5px]">{r}</span>)}
        </div>
      )}
      {ace.objectType && (
